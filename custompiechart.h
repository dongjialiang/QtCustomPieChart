...
public:
    ...
    explicit CustomPieChart(QString title, QString tag, int data,
                            QColor color, QWidget * parent = nullptr);
    explicit CustomPieChart(QString title, QList<QString> tagList, QList<int> dataList,
                            QList<QColor> colorList, QWidget * parent = nullptr);
    ~CustomPieChart();
    void addSlice(QString tag, int data, QColor color);                                // 增加切片的函数
    void setSeries(QList<QString> tagList, QList<int> value, QList<QColor> colorList); // 设置系列的函数
    void setTitleFont(QFont font);                                                     // 设置标题字体的函数
    void setTagFont(QFont font);                                                       // 设置标签字体的函数
    void setLegendFont(QFont font);                                                    // 设置说明字体的函数
    void setSumFont(QFont font);                                                       // 设置总数字体的函数
    void setSumTextFont(QFont font);                                                   // 设置"总数"文本字体的函数
    void setGlobalFont(QFont font);                                                    // 设置全局字体的函数
    void setRingSize(double ringSize);                                                 // 设置圆环大小的函数
